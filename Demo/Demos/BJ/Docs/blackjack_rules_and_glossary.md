...
- **Ход дилера.** После завершения всех рук игрока дилер вскрывает закрытую карту и добирает до жёстких 17 включительно. На мягких 17 дилер также останавливается. Если все руки игрока перебрали, дилер пропускает ход и не вскрывает карту. Дилер не делает `Hit`, если рука достигла 17 или больше.
...
- **Несовместимые рекомендации стратегии.** Если таблица советует действие, которое запрещено текущими условиями (например, `Split` после достижения лимита рук, `Double Down` или `Surrender` после взятой карты), автоигрок выбирает ближайшую допустимую альтернативу. Вместо недоступного `Double Down` и `Surrender` выполняется `Hit`, а при невозможном `Split` запрашивается рекомендация для той же руки как для несдвоенной; если и она снова равна `Split`, выполняется `Hit`.
